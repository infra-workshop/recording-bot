...
export = function costumed_blockquote(state: StateBlock, startLine: number, endLine: number, silent: boolean) {
  var adjustTab;
  var ch;
  var i;
  var initial;
  ...
  for (nextLine = startLine + 1; nextLine < endLine; nextLine++) {
    ...
    if (state.src.charCodeAt(pos++) === 0x3E/* > */&&
        state.src.charCodeAt(pos) === 0x20/* space */&& !wasOutdented) {
...
// Quirk to enforce "hard termination mode" for paragraphs;
  ...
  (state.md.block as any).tokenize(state, startLine, nextLine);
